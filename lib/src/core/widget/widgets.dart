// This barrel file exports all global reusable widgets
// allowing for simple imports across the application.

// Dialogs
export 'dialog/confirm_save_dialog.dart';
export 'dialog/confirm_delete_dialog.dart';

// Buttons
export 'button/primary_button.dart';
export 'button/outlined_button.dart';
export 'button/attendance_action_row.dart';
export 'button/hafalan_action_row.dart';
export 'button/expandable_fab_menu.dart';

// Inputs
export 'input/app_text_field.dart';

// Cards & Headers
export 'card/santri_hero_card.dart';
export 'card/attendance_legend_card.dart';
export 'card/attendance_summary_card.dart';
export 'header/santri_context_header.dart';

// Navigation
export 'navigation/app_liquid_bottom_nav.dart';

// Tab
export 'tab/app_tab_selector.dart';

// Month navigation
export 'month/app_month_selector.dart';
export 'month/app_calendar_picker_button.dart';

// Shimmer — loading skeletons
export 'shimmer/shimmer_box.dart';
export 'shimmer/shimmer_data_list_item.dart';
export 'shimmer/shimmer_halaqoh_card.dart';
export 'shimmer/shimmer_dashboard_header.dart';
export 'shimmer/shimmer_setoran_item.dart';
export 'shimmer/shimmer_halaqoh_info_card.dart';
export 'shimmer/shimmer_santri_list_item.dart';
export 'shimmer/shimmer_detail_santri.dart';
export 'shimmer/shimmer_absensi_santri_item.dart';
export 'shimmer/shimmer_detail_absensi_hari_ini.dart';