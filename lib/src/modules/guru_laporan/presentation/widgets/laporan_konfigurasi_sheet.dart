...
import '../cubits/laporan_absensi_cubit.dart';
import '../cubits/laporan_absensi_state.dart';

/// Redesigned bottom sheet for configuring and generating an attendance PDF.
///
/// Changes from v1:
///  - Card-based range type selector (Mingguan / Bulanan / Kustom)
///  - Mingguan: week-strip calendar — tap any week in a month
///  - Bulanan: scrollable month-grid picker
///  - Kustom: independent start + end date pickers (any range)
///  - Live preview bar showing selected date range
///  - Animated section transitions
class LaporanKonfigurasiSheet extends StatefulWidget {
  ...
  static void show(
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => BlocProvider(
        create: (_) => sl<LaporanAbsensiCubit>(),
        child: LaporanKonfigurasiSheet(
          records: records,
  ...
  @override
  State<LaporanKonfigurasiSheet> createState() =>
      _LaporanKonfigurasiSheetState();
}

class _LaporanKonfigurasiSheetState extends State<LaporanKonfigurasiSheet>
  ...
  // ── Weekly ────────────────────────────────────────────────────────────────
  late int _weekMonth; // month being displayed in the week strip
  late int _weekYear;
  ...
  // ── Config ────────────────────────────────────────────────────────────────
  LaporanAbsensiConfig get _config => LaporanAbsensiConfig(
  ...
  String get _filename {
    ...
    return 'Laporan_Absensi_${slug}_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf';
  }
  ...
  // Returns list of week-start Mondays for a given month
  ...
  Future<void> _pickDate(bool isStart) async {
    ...
    setState(() {
      if (isStart) {
        ...
        _customEnd = picked;
        // Auto-adjust start if it's after end
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<LaporanAbsensiCubit, LaporanAbsensiState>(
  ...
  Widget _buildConfigState(
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 4.h),
        Text(
          t.laporanConfig.subtitleAbsensi,
          style: TextStyle(
            fontSize: 12.sp,
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            onPressed: isGenerating || !_isCustomValid
                ? null
                : () => context.read<LaporanAbsensiCubit>().generatePdf(
                    _config,
                    widget.records,
                  ),
      ...
      children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      t.laporanConfig.attendanceReport,
                      style: TextStyle(
                        fontSize: 13.sp,
                        fontWeight: FontWeight.w600,
                        color: colors.textPrimary,
          ...
          children: [
            Expanded(
              child: _ActionButton(
                ...
                isPrimary: false,
                colors: colors,
                onTap: () => context.read<LaporanAbsensiCubit>().previewPdf(
                  pdfBytes as dynamic,
                  _filename,
                ),
              ),
            ...
            Expanded(
              child: _ActionButton(
                ...
                isPrimary: true,
                colors: colors,
                onTap: () => context.read<LaporanAbsensiCubit>().sharePdf(
                  pdfBytes as dynamic,
                  _filename,
                ),
              ),
        ...
        Center(
          child: TextButton.icon(
            onPressed: () => context.read<LaporanAbsensiCubit>().reset(),
            icon: Icon(
